...
- A pagina de `Importacao` agora permite enviar um XLSX e valida apenas a estrutura do arquivo, conferindo extensao/formato, existencia das abas `Modelo` e `Instruções` e cabecalhos oficiais da aba `Modelo`; nesta fase, nenhum lancamento e gravado no banco.
- Ficou definido apenas em nivel de direcao para a proxima fase da importacao real que a primeira versao deve usar somente cadastros ja existentes, nao criar automaticamente pessoas/categorias/contas/centros de custo, nao gravar nada se houver qualquer erro, e deixar preview, erros por linha/campo, importacao de cadastros auxiliares e eventual importacao parcial apenas para fases futuras.
