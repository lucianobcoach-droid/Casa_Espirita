...
- A exportacao real de lancamentos em XLSX passou a ficar operacional na propria tela de `Lancamentos financeiros`, com a acao `Exportar` respeitando os filtros GET ativos da listagem e gerando uma aba `Lancamentos` com cabecalhos amigaveis ao usuario, na mesma ordem do modelo de importacao; a pagina separada agora fica visualmente focada em importacao futura e download da planilha modelo, com atalho de retorno para a listagem filtravel.
