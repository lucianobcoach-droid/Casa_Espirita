...
- O retorno visual da importacao agora destaca o resultado em um banner superior de sucesso ou erro, mantem os totais em cards de leitura rapida, apresenta cada linha com erro em blocos mais escaneaveis e, quando ha inconsistencias, oferece download de relatorio XLSX com `Linha`, `Campo`, `Mensagem` e `Como corrigir`, sem alterar a politica all-or-nothing nem expor nomes tecnicos de campo para o usuario final.
