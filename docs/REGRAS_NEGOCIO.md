...
- No `Extrato Financeiro`, a evolucao futura da exportacao XLSX deve separar dois modos: `Exportacao padrao`, espelhando layout/colunas/ordem visiveis na tela, e `Exportacao completa`, reunindo os campos operacionais/documentais uteis ja disponiveis no contexto exportado.
- Enquanto a frente de reabertura do `financeiro` permanecer prioritaria, o app `biblioteca` deve continuar tratado como backlog congelado, salvo decisao explicita em contrario.
...
- Em documentos de `recibos` e `termos`, itens da mesma subcategoria documental podem ser agrupados no mesmo documento com soma dos valores; itens de categorias pai diferentes devem permanecer em documentos separados.
- Quando houver subcategorias diferentes sob a mesma categoria pai em `recibos` ou `termos`, a decisao entre agrupar pela categoria pai com detalhamento opcional ou manter separacao por subcategoria depende de SPEC propria antes de qualquer implementacao.
