...
- filtros, configuracoes de colunas e opcoes visuais secundarias podem ficar em paineis compactos/recolhiveis quando a tela ficaria pesada demais; esses paineis devem vir recolhidos por padrao, e quando houver filtros ou opcoes ativas a tela deve sinalizar isso de forma discreta sem esconder acoes essenciais
- em telas analiticas com KPIs e area forte de resultados, o painel de filtros deve permanecer no topo da area de analise, no mesmo lugar estrutural quando estiver resumido ou expandido; o estado recolhido deve mostrar resumo compacto da configuracao atual, sem empurrar o filtro para baixo dos resultados
...
- na `lancamento_list`, acoes documentais redundantes devem ser evitadas: quando a diferenca for apenas o agrupamento automatico por favorecido, a interface deve preferir uma unica acao visivel de `Recibos em lote` e uma unica acao visivel de `Termo anual de quitacao`
- em documentos anuais do financeiro, a assinatura institucional deve reaproveitar o mesmo estilo manuscrito ja consolidado no recibo oficial, sem criar assinatura paralela
